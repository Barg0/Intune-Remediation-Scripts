...
foreach ($printerPath in $sharedPrinters) {
    try {
        ...
        if (-not $matchedPrinter) {
            ...
            Write-Log "Printer '$printerPath' is already installed, skipping." -Tag "Info"
        }
    }
    catch {
...
foreach ($printerPath in $sharedPrinters) {
    Write-Log "Verifying printer: $printerPath" -Tag "Debug"
    $matchedPrinter = $installedPrinters | Where-Object { $_.Name -eq $printerPath -or $_.ShareName -eq $printerPath }
    ...
    if ($matchedPrinter) {
        ...
        Write-Log "Printer '$printerPath' is still missing after remediation." -Tag "Error"
