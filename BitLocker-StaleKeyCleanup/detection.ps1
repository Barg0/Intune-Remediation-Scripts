...
try {
    # ---------------------------[ Prerequisites: Entra Joined ]---------------------------
    Write-Log "Checking MDM/Entra enrollment status..." -Tag "Get"
    $mdmEnrollment = Get-ItemProperty -Path "HKLM:\SOFTWARE\Microsoft\Enrollments\*" -ErrorAction SilentlyContinue |
        Where-Object { $_.UPN } | Select-Object -First 1

    Write-Log "MDM enrollment found: $($null -ne $mdmEnrollment)" -Tag "Debug"
    if ($mdmEnrollment) {
        Write-Log "Enrollment UPN: $($mdmEnrollment.UPN)" -Tag "Debug"
    }
    ...
    foreach ($path in @($pathCommon, $pathTenant)) {
        ...
        if ($path -eq $pathCommon -and $pathTenant) {
            ...
            Write-Log "GET failed after $bitLockerApiMaxRetries attempt(s) - detection cannot complete (non-compliant)" -Tag "Debug"
            Complete-Script -ExitCode 1
        }
    }
    if (-not $response) {
    ...
    foreach ($keyItem in $keys) {
        ...
        if ($kid) {
            ...
            Write-Log "Key $normalizedKid - Drive type: $volLabel" -Tag "Debug"
    ...
    Write-Log "Total keys in Entra: $($keys.Count) | Current KIDs to keep: $($currentKids.Count) | Orphaned OS keys to remove: $($staleOsKids.Count)" -Tag "Debug"

    if ($staleOsKids.Count -gt 0) {
        Write-Log "Detected $($staleOsKids.Count) orphaned OS key(s) - remediation needed (non-compliant)" -Tag "Info"
        Complete-Script -ExitCode 1
    }

    Write-Log "No orphaned OS keys found - compliant" -Tag "Success"
