...
# ---------------------------[ Script name ]---------------------------
$scriptName = "Deploy - Network Drive Labels"
$logFileName = "deploy.log"

# ---------------------------[ Intune Remediation Config ]---------------------------

# Display name prefix -- each remediation will be named: "{prefix} - {DriveLetter}"
$intuneNamePrefix = "🏷️ Network Drive - Label"
...
$scriptsDir = "$PSScriptRoot\label-scripts"
...
if (-not (Test-Path $scriptsDir)) {
    Write-Log "label-scripts/ directory not found. Run package.ps1 first." -Tag "Error"
    Complete-Script -ExitCode 1
}
...
foreach ($entry in $driveEntries) {
    ...
    if ([string]::IsNullOrWhiteSpace($driveLetter) -or
        ...
        [string]::IsNullOrWhiteSpace($driveLabel) -or
        [string]::IsNullOrWhiteSpace($entraGroup)) {
        Write-Log "Skipping incomplete row: DriveLetter='$driveLetter' EntraGroup='$entraGroup'" -Tag "Error"
        $failed++
        continue
    }

    $intuneName = "$intuneNamePrefix - $driveLetter"
    $intuneDescription = "Sets the label '$driveLabel' on drive $driveLetter`: mapped via ADMX."

    Write-Log "--- Processing drive $driveLetter`: '$driveLabel' -> Group '$entraGroup' ---" -Tag "Run"

    # ---------------------------[ Read Packaged Scripts ]---------------------------
    $detectionPath   = "$scriptsDir\$driveLetter\detection.ps1"
    $remediationPath = "$scriptsDir\$driveLetter\remediation.ps1"

    if (-not (Test-Path $detectionPath) -or -not (Test-Path $remediationPath)) {
        Write-Log "Scripts not found in $scriptsDir\$driveLetter\. Run package.ps1 first." -Tag "Error"
        $failed++
        continue
    }
    ...
    Write-Log "Scripts loaded and encoded for drive $driveLetter`." -Tag "Get"
...
# ---------------------------[ Summary ]---------------------------
Write-Log "Deployed: $deployed | Failed: $failed | Total: $($driveEntries.Count)" -Tag "Info"

if ($failed -eq 0) {
    Write-Log "All drive label remediations deployed successfully." -Tag "Success"
    Complete-Script -ExitCode 0
} else {
    Write-Log "Some drive label remediations failed. Check the log above for details." -Tag "Error"
    Complete-Script -ExitCode 1
}
