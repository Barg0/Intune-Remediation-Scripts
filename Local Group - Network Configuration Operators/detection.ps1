...
try {
    # 1. Get the logged-on user
    $loggedOnUser = (Get-WmiObject -Class Win32_ComputerSystem).UserName
    if (-not $loggedOnUser) {
        Write-Log "No logged-on user detected." -Tag "Info"
        Complete-Script -ExitCode 0
    }

    # 2. Translate username to SID
    try {
    ...
    # 4. Get members by SID
    $memberSIDs = Get-LocalGroupMember -Group $groupName | Select-Object -ExpandProperty SID
    Write-Log "Group '$groupName' member SIDs: $($memberSIDs -join ', ')" -Tag "Debug"

    # 5. Compare SID
    if ($memberSIDs -contains $loggedOnUserSID) {
        Write-Log "User '$loggedOnUser' is already in group '$groupName'." -Tag "Success"
        Complete-Script -ExitCode 0
    } else {
        Write-Log "User '$loggedOnUser' is NOT in group '$groupName'." -Tag "Error"
        Complete-Script -ExitCode 1
    }

} catch {
    Write-Log "Unexpected error during detection: $_" -Tag "Error"
    Complete-Script -ExitCode 1
}